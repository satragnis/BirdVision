...
class MultiObjectProcessor(
    ...
    private val workflowModelViewModel: WorkflowModelViewModel,
    ...
    override fun onSuccess(
        ...
        var objects = results
        if (!workflowModelViewModel.isCameraLive) {
            return
        }
        ...
        if (selectedObject != null) {
            workflowModelViewModel.confirmingObject(selectedObject, confirmationController.progress)
        } else {
            workflowModelViewModel.setWorkflowState(
                if (objects.isEmpty()) {
                    WorkflowModelViewModel.WorkflowState.DETECTING
                } else {
                    WorkflowModelViewModel.WorkflowState.DETECTED
                }
            )
        }
    }
